...
extern int8_t mb_write_hold_multipe_register(mb_master_t *ptMaster, uint8_t chSlaveNumber, uint16_t hwDataAddr, const uint16_t *phwBuf, uint16_t hwDataNumber, uint32_t wTimeout);
...
extern int8_t mb_do_request(mb_master_t* ptMaster, const mb_request_t* ptRequest);
